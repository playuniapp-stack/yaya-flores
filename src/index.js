...
async function pbkdf2(password,salt){const key=await crypto.subtle.importKey("raw",enc.encode(password),{name:"PBKDF2"},false,["deriveBits"]);const bits=await crypto.subtle.deriveBits({name:"PBKDF2",hash:"SHA-256",salt:enc.encode(salt),iterations:10000},key,256);return b64url(new Uint8Array(bits))}
...
async function createSession(env){const token=crypto.randomUUID()+"-"+crypto.randomUUID();await setSetting(env,"adminSessionToken",token);await setSetting(env,"adminSessionExpires",String(Date.now()+8*60*60*1000));return token}
async function authenticated(req,env){const token=cookies(req).yaya_admin;if(!token)return false;const saved=await setting(env,"adminSessionToken"),expires=Number(await setting(env,"adminSessionExpires")||0);return !!saved&&token===saved&&Date.now()<expires}
